...
open class TagListView: UIView {
    ...
    @IBInspectable open dynamic var enableImageIcon: Bool = false {
        didSet {
            defer { rearrangeViews() }
            tagViews.forEach {
                $0.enableImageIcon = enableImageIcon
            }
        }
    }
    
    @IBInspectable open dynamic var imageIconSize: CGFloat = 12 {
        didSet {
            defer { rearrangeViews() }
            tagViews.forEach {
                $0.imageIconSize = imageIconSize
            }
        }
    }
