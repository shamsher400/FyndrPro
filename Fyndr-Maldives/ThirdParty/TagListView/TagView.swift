...
//    }
    ...
    @IBInspectable open var paddingY: CGFloat = 5 {
        didSet {
    ...
    @IBInspectable open var enableImageIcon: Bool = false {
        didSet {
            imageIcon.isHidden = !enableImageIcon
            updateLeftInsets()
        }
    }
    
    @IBInspectable open var imageIconSize: CGFloat = 12 {
        didSet {
            updateLeftInsets()
        }
    }
    ...
    override open var intrinsicContentSize: CGSize {
        ...
        size.height = size.height + paddingY * 2  //textFont.pointSize + paddingY * 2
        ...
        if enableImageIcon {
            size.width += imageIconSize + paddingX
        }
    ...
    private func updateLeftInsets() {
        if enableImageIcon {
            titleEdgeInsets.left = paddingX  + imageIconSize + paddingX
        }
        else {
            titleEdgeInsets.left = paddingX
        }
    ...
    open override func layoutSubviews() {
        ...
        if enableImageIcon {
            imageIcon.frame = CGRect(x: paddingX, y: (self.frame.height - imageIconSize)/2 , width: imageIconSize, height: imageIconSize)
        }
    }
}
